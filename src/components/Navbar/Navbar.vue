...
<template>
  <div class="flex flex-row w-full justify-between items-center sm:gap-8 gap-4 py-4 mb-4 lg:px-8 px-3 bg-white">
    <div class="flex flex-row justify-start items-center gap-x-3">
      <img src="/icon/CUPSAA.png" class="w-10 h-10" />
      <div class="font-bold text-[20px] hidden">CUPSAA</div>
    </div>
    ...
    <div class="flex flex-row justify-start items-center gap-x-2 sm:text-[16px] text-[14px] font-bold" v-else>
      <div>{{ getFirstName }}</div>
      <div>|</div>
      <div class="flex flex-row justify-start items-center gap-x-1 sm:text-[16px] text-[14px]">
      ...
      <button v-if="isUserLogin" @click="logout" class="sm:ml-4 px-4 py-2 bg-red-500 text-white font-bold rounded-[12px] sm:text-[16px] text-[14px]">Logout</button>
    </div>
  </div>
</template>
