...
<template>
  <div class="flex flex-col justify-start items-center py-8 lg:px-32 px-3 min-h-screen relative">
    ...
    <div class="font-bold lg:text-2xl text-[20px] leading-7">ค้นหาเพื่อนจากชื่อ หรือ นามสกุล</div>
    ...
    <div class="flex flex-col justify-center items-center gap-y-4 my-3 lg:w-[80%] w-full px-3">
      ...
      <div v-if="isEmptyFriend" class="w-full flex flex-col justify-center items-center">
        <div class="sm:text-[20px] text-[16px] font-bold">ไม่พบเพื่อนที่มีชื่อตรงกับที่คุณกรอก</div>
      </div>
      <div v-for="friend in friends" v-bind:key="friend.id" class="w-full">
        <div class="flex justify-between items-center w-full py-4 rounded-xl">
          <div class="lg:w-[250px] w-[200px] sm:text-[16px] text-[14px]">{{ friend.fullName }}</div>
          <div class="">{{ friend.generationId }}</div>
          <router-link :to="{ path: `view-friend/${friend.id}` }">
            <button class="rounded-xl lg:px-8 px-4 py-2 bg-blue-500 text-white font-semibold hover:bg-opacity-80 transition-colors duration-200 sm:text-[16px] text-[14px]">View</button>
          </router-link>
        </div>
      </div>
    </div>
    <button class="fixed bottom-[32px] sm:right-[64px] right-5 w-12 h-12 rounded-full bg-blue-500 flex justify-center items-center" @click="scrollToTop">
